...
const MenuItem = (props) => {
    const {title, iconClass, type, path, badgeCount, children: links, name, collapseId, handleNavClick} = props;
                                            ...
                                            {link.children ? (
                                                    ...
                                                    <ul className={`collapse  ${link.children.some(link => isActive(link.path)) ? "show" : ""}`}
                                                        ...
                                                        {link.children.map((underLink, index) => (
                                                            <li key={index}
                                                                ...
                                                                <Link className="" to={underLink.path} onClick={handleNavClick}>
                                                                    {underLink.name}
                                                                </Link>
                                                            </li>
                                                        ))}
                                                ...
                                                <Fragment>
                                                    <li key={index} className={isActive(link.path) ? 'active' : ''}>
                                                        <Link to={link.path} onClick={handleNavClick}>{link.name}</Link>
                                                    </li>
                                                </Fragment>
                                            )}
                                        </Fragment>
