...
const DesignationMaster = () => {
    ...
    // RESPONSIVE STATE (Crucial: This defines 'isMobile')
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (isEdit) {
                await updateDesignation(form.id, form);
            } else {
    ...
    const handleEdit = (designation) => {
        setForm({ ...designation, id: designation._id });
        setIsEdit(true);
        setShowForm(true);
        window.scrollTo({ top: 0, behavior: "smooth" });
    };
    ...
    const confirmDelete = async () => {
        ...
        try {
            await deleteDesignation(deleteTarget._id);
            fetchDesignations();
            ...
            <div style={styles.container}>
                ...
                <div style={styles.listCard}>
                    ...
                    <div className="d-block d-md-none premium-mobile-cards-container">
                        {filteredDesignations.length === 0 ? (
                            <div style={styles.noData}>No designations found.</div>
                        ) : (
                            filteredDesignations.map((d) => (
                                <div key={d._id} className="premium-mobile-card" onClick={() => handleView(d)}>
                                    <div className="card-header-row" style={{justifyContent: 'space-between', alignItems: 'center'}}>
                                        <div className="card-title-container">
                                            <h4 className="card-company-name">{d.name}</h4>
                                        </div>
                                        <div style={{display: 'flex', alignItems: 'center', gap: '8px'}}>
                                            <span
                                                style={{
                                                    ...
                                                    fontSize: '12px',
                                                    fontWeight: 700,
                                                    color: d.status === "Active" ? "#065f46" : "#9f1239",
                                                    backgroundColor: d.status === "Active" ? "#d1fae5" : "#fecdd3",
                                                    marginRight: '4px'
                                                }}>
                                                {d.status}
                                            </span>
                                            <button className="small-action-btn btn-edit" onClick={(e) => { e.stopPropagation(); handleEdit(d); }}>
                                                <EditIcon />
                                            </button>
                                            <button className="small-action-btn btn-delete" onClick={(e) => { e.stopPropagation(); handleDeleteClick(d); }}>
                                                <DeleteIcon />
                                            </button>
                                        </div>
                                    </div>
