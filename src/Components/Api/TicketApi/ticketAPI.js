...
export const getTickets = async (assignedOnly = false) => {
    try {
        ...
        if (response.data && response.data.data) {
            const ticketsWithImages = response.data.data.filter(t => t.image);
            if (ticketsWithImages.length > 0) {
                console.log("🖼️ Tickets with images found:", ticketsWithImages.length);
                ticketsWithImages.slice(0, 3).forEach((t, i) => {
                    console.log(`   Ticket ${i+1}: image="${t.image}"`);
                });
            }
        }
