            ...
            <div style={styles.container}>
                ...
                <div style={styles.listCard}>
                    ...
                    <div className="d-block d-md-none premium-mobile-cards-container">
                        {filteredPriorities.length === 0 ? (
                            <div style={styles.noData}>No priorities found.</div>
                        ) : (
                            filteredPriorities.map((p) => (
                                <div key={p._id} className="premium-mobile-card" onClick={() => handleView(p)}>
                                    <div className="card-header-row" style={{justifyContent: 'space-between', alignItems: 'center'}}>
                                        <div style={{display: 'flex', alignItems: 'center'}}>
                                            <div className="card-logo-container" style={{backgroundColor: p.color}}>
                                                {/* Just use the background color as the logo representation */}
                                            </div>
                                            <div className="card-title-container">
                                                <h4 className="card-company-name">{p.name}</h4>
                                                <span className="card-company-code">{p.color}</span>
                                            </div>
                                        </div>
                                        <div style={{display: 'flex', alignItems: 'center', gap: '8px'}}>
                                            <span
                                                style={{
                                                    ...
                                                    fontSize: '12px',
                                                    fontWeight: 700,
                                                    color: p.status === "Active" ? "#065f46" : "#9f1239",
                                                    backgroundColor: p.status === "Active" ? "#d1fae5" : "#fecdd3",
                                                    marginRight: '4px'
                                                }}>
                                                {p.status}
                                            </span>
                                            <button className="small-action-btn btn-edit" onClick={(e) => { e.stopPropagation(); handleEdit(p); }}>
                                                <EditIcon />
                                            </button>
                                            <button className="small-action-btn btn-delete" onClick={(e) => { e.stopPropagation(); handleDeleteClick(p); }}>
                                                <DeleteIcon />
                                            </button>
                                        </div>
                                    </div>
