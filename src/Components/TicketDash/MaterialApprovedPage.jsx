...
} from "@/Components/Api/TicketApi/workLogAPI";
...
const MaterialApprovedPage = () => {
    ...
    try {
      ...
      if (saved) {
        ...
        Object.keys(parsed).forEach((key) => {
          restored[key] = {
            ...parsed[key],
            startTime: parsed[key].startTime
              ? new Date(parsed[key].startTime)
              : null,
            endTime: parsed[key].endTime ? new Date(parsed[key].endTime) : null,
            systemStartTime: parsed[key].systemStartTime
              ? new Date(parsed[key].systemStartTime)
              : null,
          };
        });
        return restored;
  ...
  // Mixed Auto-Timer & Manual Timer calculation
  const getAnalysisTimerData = (analysis, current_time) => {
    if (!analysis)
      return {
        startTime: null,
        endTime: null,
    ...
    const storedTimer = timers[analysisId];

    // Use the WorkAnalysis's own approved_at — this is set by the backend
    // when the manager approves (updateApproval controller line 230).
    // Do NOT use analysis.ticket_id?.approved_at — that field belongs to
    // the ticket's own approval workflow and is always null here.
    const approvedAtStr =
      ...
      analysis.updatedAt ||
      analysis.createdAt;
    const realApprovedAt = approvedAtStr ? new Date(approvedAtStr) : new Date();

    if (storedTimer) {
      // If the ticket was re-approved after the timer was stopped, the stored timer is stale.
      // Ignore it so the auto-timer can restart from the new approval time.
      const isStale = storedTimer.endTime && new Date(storedTimer.endTime) < realApprovedAt;

      if (!isStale) {
        // If this was an auto-start timer, always recalculate from approved_at
        // (in case the stored time was wrong before the backend fix)
      if (storedTimer.isAutoStart && storedTimer.isRunning) {
        return {
          ...storedTimer,
          startTime: realApprovedAt,
          duration: calculateDuration(realApprovedAt, current_time || new Date()),
        };
      }
  ...
  // NO stored timer (or it was stale) = First time! Auto-start from approved_at
    return {
      startTime: realApprovedAt,
      endTime: null,
      isRunning: true,
  ...
  const [selectedAnalysisForCompletion, setSelectedAnalysisForCompletion] =
    useState(null);
  ...
  const fetchWorkAnalyses = async () => {
    try {
      ...
      try {
        ...
        const ticketsArray = Array.isArray(allTicketsRes)
          ? allTicketsRes
          : allTicketsRes.data || [];
        const ticketMap = new Map(
          (ticketsArray || []).map((t) => [String(t._id), t]),
        );
        ...
        // Clear stale auto-start timers so they recalculate from real approved_at
        setTimers((prev) => {
          const cleaned = { ...prev };
          enriched.forEach((a) => {
            const stored = cleaned[a._id];
            if (stored && stored.isAutoStart && stored.isRunning) {
              // Remove stale auto-start — getAnalysisTimerData will rebuild it
              // from analysis.approved_at on next render
              delete cleaned[a._id];
            }
          });
          return cleaned;
        });
  ...
  const calculateTotalLoggedTime = () => {
    ...
    workAnalyses.forEach((analysis) => {
      workLogs.forEach((log) => {
        const logMatches =
          String(log.analysis_id) === String(analysis._id) &&
  ...
  const calculateTicketTotalTime = (analysisId) => {
    ...
    workLogs.forEach((log) => {
      const logMatches =
        String(log.analysis_id) === String(analysisId) &&
        ((log.worker_name || "").toLowerCase().trim() === currentUserName ||
          String(log.worker_id || "") === String(user?.id || ""));
    ...
    const currentTimer = getAnalysisTimerData(
      workAnalyses.find(a => String(a._id) === String(analysisId)),
      currentTime
    );
    if (currentTimer && currentTimer.duration && currentTimer.isRunning) {
        const durationStr = String(currentTimer.duration || "").trim();
        const durationParts = durationStr.match(/(\d+)h\s*(\d+)m\s*(\d+)s/i);
        if (durationParts && durationParts.length >= 4) {
          const hours = parseInt(durationParts[1]) || 0;
          const minutes = parseInt(durationParts[2]) || 0;
  ...
  const calculateDuration = (fromTime, toTime) => {
    ...
    const fromDate =
      typeof fromTime === "string" ? new Date(fromTime) : fromTime;
  ...
  const handleEndTimer = async () => {
    ...
    const analysisId = selectedAnalysisForLog._id;
    const now = new Date();
    const timerData = getAnalysisTimerData(selectedAnalysisForLog, now);
  ...
  const handleSubmitWorkLogAuto = async (timerData) => {
    ...
    try {
      ...
      const startTimeDate =
        timerData.startTime instanceof Date
          ? timerData.startTime
      ...
      const fromTimeStr =
        startTimeDate.getHours().toString().padStart(2, "0") +
        ":" +
      ...
      // DO NOT update ticket status to "Working In Progress" automatically!
      // This allows the user to start and stop the timer multiple times 
      // (loop based) without the ticket disappearing from this page.
      ...
      setTimers((prev) => {
        ...
        // Do not delete the timer here, so it stays stopped visually
        return newTimers;
      });
  ...
  const handleSubmitWorkLog = async () => {
    ...
    const analysisId = selectedAnalysisForLog._id;
    // We check timers first because a stopped timer is only in the timers state
    const timerData = timers[analysisId] || getAnalysisTimerData(selectedAnalysisForLog, new Date());
    ...
    try {
      // Format time strings (HH:MM)
      const fromTimeStr =
        timerData.startTime.getHours().toString().padStart(2, "0") +
        ":" +
        timerData.startTime.getMinutes().toString().padStart(2, "0");
      const toTimeStr =
        timerData.endTime.getHours().toString().padStart(2, "0") +
        ":" +
  ...
  const handleSubmitCompleteWork = async () => {
    try {
      ...
      if (statusesToSearch && statusesToSearch.length > 0) {
        statusObj = statusesToSearch.find(
          (s) =>
            String(s.name).toLowerCase() === String(statusName).toLowerCase(),
        );
        statusId = statusObj?._id || statusObj?.id || null;
      }

      const updatePayload = statusId
        ? { status_id: statusId }
        : { status: statusName };
    ...
    <div style={styles.pageContainer}>
      ...
      {selectedAnalysisToView && (
        <div
          ...
          onClick={() => setSelectedAnalysisToView(null)}
        >
            ...
            <div style={styles.modalBody}>
                ...
                >
                  <div>
                    ...
                    <p style={styles.detailValue}>
                      {selectedAnalysisToView.ticket_id?.ticket_id || "-"}
                    </p>
                  </div>
                  <div>
                    <h5 style={styles.detailLabel}>Title</h5>
                    <p style={styles.detailValue}>
                      {selectedAnalysisToView.ticket_id?.title || "-"}
                    </p>
                  </div>
                  <div>
                    <h5 style={styles.detailLabel}>Department</h5>
                    <p style={styles.detailValue}>
                      {selectedAnalysisToView.ticket_id?.department_id?.name ||
                        "-"}
                    </p>
                  </div>
                  <div>
                    ...
                    <p style={styles.detailValue}>
                      {selectedAnalysisToView.ticket_id?.company_id?.name ||
                        "-"}
                    </p>
                  </div>
                  <div>
                    ...
                    >
                      <button
                        ...
                        onClick={() =>
                          openMap(
                            selectedAnalysisToView.ticket_id?.location ||
                              selectedAnalysisToView.location ||
                              selectedAnalysisToView.ticket_location,
                          )
                        }
                        title={
                          selectedAnalysisToView.ticket_id?.location ||
                  ...
                  <div>
                    ...
                    <p style={styles.detailValue}>
                      {selectedAnalysisToView.ticket_id?.priority_id?.name ||
                        "-"}
                    </p>
                  </div>
                </div>
              ...
              >
                ...
                >
                  <div>
                    ...
                    <p style={styles.detailValue}>
                      {selectedAnalysisToView.analysis_id ||
                        selectedAnalysisToView._id}
                    </p>
                  </div>
                  <div>
                    <h5 style={styles.detailLabel}>Material Required</h5>
                    <p style={styles.detailValue}>
                      {selectedAnalysisToView.material_required || "-"}
                    </p>
                  </div>
              ...
              >
                ...
                >
                  <div>
                    ...
                    {(() => {
                      const ticketStatus =
                        selectedAnalysisToView.ticket_id?.status_id?.name ||
                        selectedAnalysisToView.ticket_id?.status ||
                        "Unknown";
                      return (
                        <span
                          style={{
                  ...
                  <div>
                    ...
                    <p style={styles.detailValue}>
                      {user?.name || selectedAnalysisToView.worker_name || "-"}
                    </p>
                  </div>
                </div>
              </div>
              ...
              >
                <div>
                  ...
                  <p style={styles.detailValue}>
                    {new Date(
                      selectedAnalysisToView.created_at ||
                        selectedAnalysisToView.createdAt,
                    ).toLocaleString()}
                  </p>
                </div>
                <div>
                  ...
                  <p style={styles.detailValue}>
                    {new Date(
                      selectedAnalysisToView.updated_at ||
                        selectedAnalysisToView.updatedAt,
                    ).toLocaleString()}
                  </p>
                </div>
              </div>
              ...
              {selectedAnalysisToView.uploaded_images &&
                selectedAnalysisToView.uploaded_images.length > 0 && (
                  <div
                    style={{
                      background: "#f8fafc",
                      padding: "20px",
                  ...
                  >
                    ...
                    >
                      🖼️ Uploaded Images (
                      {selectedAnalysisToView.uploaded_images.length})
                    </h4>
                    <div
                      style={{
                        display: "grid",
      ...
      {showWorkLogForm &&
        selectedAnalysisForLog &&
        (() => {
          const analysisId = selectedAnalysisForLog._id;
          const timerData = getAnalysisTimerData(selectedAnalysisForLog, currentTime);
          
          return (
            <div style={styles.modalOverlay}>
              <div
                style={styles.modalMedium}
                ...
                <div style={styles.modalHeader}>
                  <div>
                    ...
                    >
                      Ticket:{" "}
                      <strong>
                        {selectedAnalysisForLog.ticket_id?.ticket_id}
                      </strong>{" "}
                      <span style={{ marginLeft: 8 }}>
                        | Worker:{" "}
                        <strong>
                          {user?.name || selectedAnalysisForLog.worker_name}
                        </strong>
                      </span>
                    </p>
                  </div>
                  ...
                  >
                    ...
                    <div
                      style={{
                        ...
                        border: "1px solid #e2e8f0",
                        ...(timerData.isRunning
                          ? { borderColor: "#86efac", borderWidth: "2px" }
                          : {}),
                      }}
                      className={
                        timerData.isRunning ? "timer-running-pulse" : ""
                      }
                    >
                      {timerData.isRunning && (
                        <div
                          className="timer-running-glow"
                          style={{
                            marginBottom: "16px",
                            padding: "12px 16px",
                            background: "#dcfce7",
                        ...
                        <div>
                          ...
                          <p
                            style={{
                              ...
                              fontSize: "18px",
                              fontWeight: "700",
                              color: timerData.startTime
                                ? "#10b981"
                                : "#94a3b8",
                            }}
                          >
                            {timerData.startTime
                              ? timerData.startTime.toLocaleTimeString()
                              : "-- : -- : --"}
                          </p>
                          <p
                            style={{
                          ...
                          >
                            {timerData.startTime
                              ? timerData.startTime.toLocaleDateString()
                              : "Not started"}
                          </p>
                        </div>
                        <div>
                          ...
                          <p
                            style={{
                              ...
                              fontSize: "18px",
                              fontWeight: "700",
                              color: timerData.endTime ? "#ef4444" : "#94a3b8",
                            }}
                          >
                            {timerData.endTime
                              ? timerData.endTime.toLocaleTimeString()
                              : "-- : -- : --"}
                          </p>
                          <p
                            style={{
                          ...
                          >
                            {timerData.endTime
                              ? timerData.endTime.toLocaleDateString()
                              : "Not ended"}
                          </p>
                        </div>
                      </div>
                    ...
                    >
                      {!timerData.isRunning ? (
                        // Not running (never started or was stopped): Show Start button
                        <button
          ...
          <div style={styles.modalMedium} onClick={(e) => e.stopPropagation()}>
            <div style={styles.modalHeader}>
              <div>
                ...
                >
                  Ticket:{" "}
                  <strong>{selectedAnalysisForLog.ticket_id?.ticket_id}</strong>
                </p>
              </div>
              <button
              ...
              {loadingLogs ? (
                ...
                (() => {
                  ...
                  const filteredLogs = workLogs.filter(
                    (l) =>
                      (l.worker_name || "").toLowerCase().trim() ===
                        currentUserName ||
        ...
        {filteredAnalyses.length === 0 ? (
          ...
          <div style={styles.cardGrid}>
            {filteredAnalyses.map((analysis) => {
              const statusName =
                analysis.ticket_id?.status_id?.name ||
                analysis.ticket_id?.status;
              const statusStyles = getStatusStyle(statusName);
              const timerData = getAnalysisTimerData(analysis, currentTime);
              const hasRunningTimer = timerData?.isRunning;
              const hasStoppedTimer =
                timerData &&
                !timerData.isRunning &&
                timerData.startTime &&
                timerData.endTime;
              ...
              return (
                ...
                >
                  <div style={styles.cardHeader}>
                    <div style={styles.cardIdRow}>
                      ...
                      <button
                        ...
                        onClick={() =>
                          openMap(
                            analysis.ticket_id?.location ||
                              analysis.location ||
                              analysis.ticket_location ||
                      ...
                      >
                        ...
                        >
                          ...
                          <span
                            ...
                            title={analysis.ticket_id?.title}
                          >
                            {analysis.ticket_id?.title || "-"}
                          </span>
                        </div>
                  ...
                  <div style={styles.cardBody}>
                    <div style={styles.cardInfoGrid}>
                      ...
                      <div style={styles.infoCol}>
                        ...
                        <span style={styles.value}>
                          {user?.name || analysis.worker_name || "-"}
                        </span>
                      </div>
                    </div>
                    <div style={{ ...styles.cardInfoGrid, marginTop: "12px" }}>
                      <div style={styles.infoCol}>
                        ...
                        <span
                          style={{ ...styles.statusBadge, ...statusStyles }}
                        >
                          {statusName || "Unknown"}
                        </span>
                      </div>
                    ...
                    {analysis.uploaded_images &&
                      analysis.uploaded_images.length > 0 && (
                        <div style={{ marginTop: "12px" }}>
                          <span style={styles.label}>
                            Images ({analysis.uploaded_images.length})
                          </span>
                        </div>
                      )}
                  ...
                  <div style={styles.cardFooter}>
                    ...
                    <button
                      onClick={() => {
                        setSelectedAnalysisForLog({
                          ...analysis,
                          worker_name: user?.name || analysis.worker_name,
                        });
                        setWorkLogs([]);
            ...
            <div style={styles.modalHeader}>
              <div>
                ...
                >
                  Ticket:{" "}
                  <strong>
                    {selectedAnalysisForCompletion.ticket_id?.ticket_id}
                  </strong>
                </p>
              </div>
              <button
                onClick={() => setShowCompleteWorkModal(false)}
                style={styles.iconBtn}
              >
                <CloseIcon />
              </button>
            ...
            <div style={styles.modalBody}>
              ...
              >
                ...
                <div style={{ marginBottom: "20px" }}>
                  ...
                  >
                    {user?.name ||
                      selectedAnalysisForCompletion.worker_name ||
                      "Unknown"}
                  </div>
                </div>
                ...
                <div>
                  <label style={styles.inputLabel}>
                    Upload Completion Images (Optional)
                  </label>
                  ...
                  <div style={{ position: "relative", marginBottom: "16px" }}>
                    ...
                    >
                      <div style={{ cursor: "pointer", pointerEvents: "none" }}>
                        <p style={{ margin: "0 0 6px 0", fontSize: "24px" }}>
                          📷
                        </p>
                        <p
                          style={{
